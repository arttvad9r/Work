...
Every numeric field uses `TextFieldLabelPosition.Attached(alwaysMinimize = true)`. Its label therefore remains minimized on the outline at all times, including when the field is empty and unfocused; `Время` must never fall into the input area. The duration placeholder `00:00` is independent from the label and stays centered in the empty input area.

Only the active logical slot renders the persistent editable field. Every other visible numeric slot renders a passive shell with invisible click indication, so there is no duplicate label/value and no gray rectangular ripple. If the persistent editor is not currently focused, the first tap on any passive slot both activates that logical field and focuses/shows the numeric keyboard. If it is already focused, switching logical fields does not issue another focus request and does not replace the input session.
