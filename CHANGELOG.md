...
- Segmented controls keep the single moving capsule but now add a restrained rounded press state to the touched segment; the custom state replaces the deliberately disabled rectangular framework indication without leaving the control visually silent before release. Theme mode is the explicit exception to moving selection: its capsule and label colors snap in the same frame as the global palette so the newly selected theme is never shown with the old option still highlighted.
- Theme selection is optimistic: the visible Material palette and segmented selection change atomically on tap while the serialized DataStore write completes afterward. Failed persistence rolls the palette back to repository state and surfaces error feedback instead of making the user wait on storage or another app mutation.
- Calendar and Year Summary paging share a softer pager spring and the same 35% positional snap threshold. Year arrow navigation uses the same interruptible, velocity-preserving retargeting model as month arrows. Physical-device QA showed that settle-time haptics feel delayed, so paging feedback is now gesture-synchronous instead: a swipe emits one light tick when the finger crosses the snap threshold, while a valid arrow action ticks immediately when movement starts. `settledPage` no longer emits haptics.
- Fixed calendar/year labels and the calendar's `Fill today`/monthly-summary chrome stay on the last committed period until pager settle instead of following `PagerState.currentPage` when it flips halfway through a drag. This removes the visible title/totals jump while the page is still moving.
- Year Summary keeps its vertical hierarchy but now uses about one-tenth of the viewport plus a short fade for entry/exit. Device-video QA showed that the prior one-sixteenth travel read almost as a hard cut rather than a continuous appearance.
- Full-screen Settings navigation remains symmetric but now uses about one-sixth of the viewport width with a softer navigation spring. The prior one-eighth travel was visually too abrupt on device while full-width travel remained unnecessarily heavy.
- The compact monthly summary is the actual persistent peek/header of `BottomSheetScaffold`. Vertical drags move the Material sheet directly under the finger instead of crossing a custom threshold and starting a second animation after release. The previous decorative lift/scale and threshold haptic were removed; normal tap indication and the compact closed-strip geometry remain.
- Tappable modal-sheet drag handles acknowledge touch-down with a restrained tone change instead of remaining visually silent; the calendar monthly report uses its persistent summary strip as the sheet handle instead of inserting a second handle.
...
- Settings default-rate editing swaps the read-only number and focused compact field atomically; device-video QA showed that the previous fade-through rendered both values on the same pixels for a few frames.
...
- The 4×1 widget mirrors the app's lower monthly-summary strip: the same secondary-container family, 16 dp corner language, centered month, one compact `shifts · hours · income` line with whole-ruble formatting, and a small dedicated add action; the picker preview mirrors the real layout.
...
- The full merge-candidate gate covers JVM tests, screenshot validation, debug/release lint, Android target builds, instrumented UI tests, signed-release verification and API 37 startup smoke.
- Added unit coverage for the pager haptic gate, including snap-threshold activation, vertical-gesture suppression, one-shot behavior and re-arming after returning toward the gesture origin.
- Added instrumentation coverage that protects the video-tuned restrained-but-visible Settings and Year Summary full-screen travel ranges.
