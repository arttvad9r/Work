...
- The compact monthly summary is now the actual persistent peek/header of `BottomSheetScaffold`. Vertical drags move the Material sheet directly under the finger instead of crossing a custom threshold and starting a second animation after release. The previous decorative lift/scale and threshold haptic were removed; normal tap indication and the compact closed-strip geometry remain.
...
- Settings default-rate editing now swaps the read-only number and focused compact field atomically; device-video QA showed that the previous fade-through rendered both values on the same pixels for a few frames.
